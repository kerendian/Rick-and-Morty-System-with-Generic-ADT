...
{
	if (copyFunctionKey == NULL || freeFunctionKey == NULL || printFunctionKey == NULL || equalFunctionKey == NULL || key == NULL || value == NULL
			|| copyFunctionVal == NULL || freeFunctionVal == NULL || printFunctionVal == NULL )
			return NULL;
...
Element keyValCopy(Element keyValue)
{
	...
	if(keyVal == NULL)
		return NULL;
	keyValPair newKeyVal=createKeyValuePair(keyVal->copyFunctionKey, keyVal->freeFunctionKey, keyVal->printFunctionKey, keyVal->equalFunctionKey
			, keyVal->copyFunctionVal,keyVal->freeFunctionVal, keyVal->printFunctionVal, keyVal->key, keyVal->value);
	if(newKeyVal == NULL)
		//memory problem
		return NULL;
...
bool isEqualValue (Element Value, Element valuePart){
	...
	return keyValue->equalFunctionKey(val,valuePart); //need to check  how to do  implement this when val is jerry and not the id
}
